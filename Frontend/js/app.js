...
function displayRoleIndicator() {
    ...
    const icon = currentRole === 'manager' ? 'briefcase' : 'code-2';
...
function switchRole(role) {
    ...
    newTaskBtns.forEach(btn => {
        btn.innerHTML = role === 'manager' ? '<span>+</span> New Request' : '<span>+</span> Add Task';
    });
...
window.openTaskDetail = function (taskId) {
    ...
    content.innerHTML = `
        ...
        <div class="detail-section">
            ...
            <select class="status-select" onchange="updateTaskStatusHandler('${task.id}', this.value)" ${currentRole === 'manager' && task.status !== 'review' ? 'disabled' : ''}>
